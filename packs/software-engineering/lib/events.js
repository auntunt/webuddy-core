...
// 原 ref 从 detect.js 导入；detect.js 是 ref 的事实装配层，本包不用它，
// 这个常量就地定义（内核侧的路径拼装一律走 state.js，这里只读 ref 留下的旧痕迹）。
const WEBUDDY_DIR = '.webuddy';
...
function eventsPath(projectDir) {
  return path.join(projectDir, WEBUDDY_DIR, EVENTS_FILE);
}
...
export function appendEvent(projectDir, ev) {
  try {
    const dir = path.join(projectDir, WEBUDDY_DIR);
